...
export default function AllMatchesPage() {
	...
	const updateAvgRankCell = async (cell: HTMLTableCellElement, isWingman: boolean, matchUrl: string) => {
		...
		if (matchUrl && isWingman) {
			...
			wingManRankNumber = parseInt(avgRankUrl.split('/').pop()?.split('.').shift() || '0');
		}
		...
		const rankNumber = isWingman
			? wingManRankNumber
			: parseInt(child?.src.match(/ranks\/(\d+)\.png/)?.[1] || '0');
			...
			<>
				...
				{!isFaceit && (
					<CompetitiveOrWingmanRankIcon
						...
						gamemode={isWingman ? 'Wingman' : 'Competitive'}
