...
export default function DeltaIndicator({ deltaKey, className, style, showZero }: DelatIndicatorProps) {
	...
	useEffect(() => {
		...
		setDeltaStatus(
			  (stats?.overall?.delta[deltaKey] ?? 0) > 0 ? 'up'
			: (stats?.overall?.delta[deltaKey] ?? 0) < 0 ? 'down'
			: 'same'
		);
	}, [loading, stats]);
	...
	return (
		<div className={`row text relative ${className}`} style={style}>
			{deltaStatus === 'up'   && (<span style={{ fontFamily: 'Roboto', position: 'absolute', top: '2px', left: '-15px' }}>⌃</span>)}
			{showZero !== false && deltaStatus === 'same' && (<span className="text-light">No variation</span>)}
			{deltaStatus === 'down' && (<span style={{ fontFamily: 'Roboto', fontSize: '1.1rem', position: 'absolute', top: '-5px', left: '-15px' }}>⌄</span>)}
