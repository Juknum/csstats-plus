...
export default function MostSuccessStats() {
	...
	useEffect(() => {
		...
		setSuccess(Object.values(stats.maps.overall).reduce((acc, map) => acc + map.won, 0));

		setMostSuccessful([
				...(Object.entries(stats.maps.overall) as [CS2Map, Stats['maps']['overall'][CS2Map]][])
				.map(([map, data]) => ({
					map,
					...data
				}))
					...
					<div className="col nogap" style={{ overflowY: 'scroll' }}>
						{mostSuccessful.map(({ map, won, played }) => (
							<div key={map} className="row space-between center-y underlined">
								<img src={getMapIcon(map)} height={20} width={20} />
								...
								<div className="row nogap nowrap full-width" style={{ maxWidth: '70px' }}>
									<div style={{ height: '3px', backgroundColor: (won / played * 100) > 90 && played > 5 ? 'orange' : "rgb(125, 205, 78)", width: `${(won / played * 100)}%` }}></div>
									<div style={{ height: '3px', backgroundColor: "rgba(202, 81,  81, .2)", width: `${((played - won) / played * 100)}%` }}></div>
								</div>
