			...
			content={
				<div className="col nogap full-width">
					<div className="row nowrap space-between">
						<span className="text">CLUTCH SUCCESS</span>
					</div>
					<div className="row nogap space-between full-width full-height">
						<div className="row full-width space-evenly" style={{ maxWidth: "calc(100% - 120px - 10px)" }}>
							{versus.map((type) => (
								<div key={type} className="col center-y center-x full-height relative" style={{ width: "80px" }}>
									{clutchChart(!loading && stats ? stats.overall[type] : 0, false)}
									<span className="text-light absolute">{!loading && stats ? `${stats.overall[type]}%` : "-"}</span>
						...
						<div className="border-left col center-x center-y full-height relative" style={{ width: "120px" }}>
